...
var Theme = {
    ...
    init: function () {
      ...
      if (window.innerWidth > 679) {
        ...
        root.style.setProperty(
          "--headerHeight",
          document.querySelector(".MainMenu.Fixed").clientHeight + "px"
        );
      }
      ...
      removeButtons.forEach((button) => {
        button.addEventListener("click", function () {
          Theme.changeCart(
            button.closest("[data-item]").getAttribute("data-item"),
            0
          );
        });
      });
  ...
  addToCart: function (variantID, amount) {
    ...
    fetch(window.Shopify.routes.root + "cart/add.js", {
      ...
      //   console.log(data);
        ...
        var cartErrors = document.querySelectorAll("[data-carterror]");
        cartCounters.forEach((element) => {
  ...
  rerenderCart: function () {
    fetch(window.Shopify.routes.root + "?sections=mini-cart")
      ...
      .then((data) => {
        ...
        var currentCart = node
          .querySelector("[data-cartamount]")
          .getAttribute("data-cartamount");
        var currentCartPrice = node
          .querySelector("[data-cartamount-price]")
          .getAttribute("data-cartamount-price");
  ...
  sections: {
            ...
            .addEventListener("change", function () {
              if (variants) {
                function getVariant(section) {
                  var selectedOptions = [];
                  var variants = JSON.parse(
                    popupContainer.querySelector("[data-variant-json]")
                      .innerHTML
                  );
                  var variantContainers = popupContainer.querySelectorAll(
                ...
                if (getVariant().available) {
                  ...
                  atc.previousElementSibling.style.opacity = 1
                  // atc.querySelector("[data-variant-price]").innerHTML =
                  //   Shopify.formatMoney(getVariant().price);
                } else {
                  atc.setAttribute("data-ajax-add", "");
                  atc.setAttribute("disabled", true);
                  atc.style.opacity = 0.4;
                  atc.style.cursor = 'auto'
                  atc.innerHTML = "SOLD OUT";
                  atc.previousElementSibling.style.opacity = 0.4
                }
              }
            });
    ...
    Product: function (section) {
      ...
      var cartWidth = document
        .querySelector(".MainMenu--rightItem.cart")
        .getBoundingClientRect().width;
      var shopWidth = document
        .querySelector(".MainMenu--leftItem")
        .getBoundingClientRect().width;
      if (window.innerWidth > 649) {
        var productMeta = section.querySelector("[data-meta]");
        var variantContainer = section.querySelector(
          "[data-variant-container]"
        );
        variantContainer.style.height =
          sectionHeight - Theme.helpers.convertRemToPixels(4) + "px";
      } else {
      ...
      function getVariant(section) {
        var selectedOptions = [];
        var variants = JSON.parse(
          section.querySelector("[data-variant-json]").innerHTML
        );
        var variantContainers = section.querySelectorAll(
          ".optionValuesContainer"
        );
        variantContainers.forEach((container) => {
        ...
        var currentVariant = variants.find(
          (element) =>
      ...
      var mediaCounter = section.querySelectorAll(
        ".ProductMedia--Counter .Product--Image--Anchor"
      );
      ...
      stickyATC.addEventListener("click", async function () {
        ...
        var amount = parseInt(
          section
            .querySelector(".productQty [data-value]")
  ...
  helpers: {
    ...
    openDesc: function () {
      ...
      if (desk) {
        desk.setAttribute(
          "initheihgt",
          Theme.helpers.convertRemToPixels(4) + "px"
        );
  ...
  mobile_cart: {
    open: function () {
      ...
      var contentHeights =
        document.querySelector(".MainMenu.Fixed").scrollHeight;
      document.querySelector("#cartMobile").style.height =
        "calc(95vh - " + contentHeights + "px)";
      var cartHeight = cart.scrollHeight;
      var headerHeight = document.querySelector(".MainMenu.Fixed").scrollHeight;
      let root = document.documentElement;
      root.style.setProperty(
        "--headerMobile",
        headerHeight - cartHeight + "px"
      );
